    ...
    @if (viewModel(); as vm) {
      <section class="rounded-2xl bg-slate-800/50 border border-slate-700 p-6 space-y-6">
        <div class="flex items-center justify-between gap-4">
          <h2 class="text-lg font-semibold text-white">{{ 'verificationVerdict.title' | translate }}</h2>
          <app-status-badge [label]="vm.resultLabelKey | translate" [tone]="vm.resultTone" />
        </div>
        ...
        @if (vm.evidenceBanner.visible) {
          <div
            ...
            role="status"
          >
            {{ vm.evidenceBanner.messageKey | translate }}
          </div>
        }

        @for (group of vm.groups; track group.titleKey) {
          <div>
            <h3 class="text-sm font-medium text-slate-300 mb-3">{{ group.titleKey | translate }}</h3>
            <ul class="space-y-2">
              @for (row of group.rows; track row.key) {
                <li
                  class="flex items-center justify-between rounded-lg bg-slate-900/60 px-4 py-3 text-sm"
                >
                  <span class="text-slate-300">{{ row.labelKey | translate }}</span>
                  <span [ngClass]="rowToneClass(row.tone)">
                    {{ row.displayValueKey ? (row.displayValueKey | translate) : row.displayValue }}
                  </span>
                </li>
              }
            </ul>
