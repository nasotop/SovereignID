...
Write-Host ""
Write-Host "Reporteria (reports-api con Postgres):"
Write-Host "  docker compose up -d reports-api"
Write-Host "  dotnet run --project src/reports/Reports.Api -- snapshot --from (Get-Date).AddDays(-7).ToString('yyyy-MM-dd') --to (Get-Date).ToString('yyyy-MM-dd')"
Write-Host "  (ejecutar backfill de institution_metrics_daily tras seed-dev cuando reports-api usa Persistence:Provider=Postgres)"
